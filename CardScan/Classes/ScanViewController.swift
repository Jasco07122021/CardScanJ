...
@objc public class ScanViewController: ScanBaseViewController {
    ...
    @objc static public func createViewController(withDelegate delegate: ScanDelegate? = nil,locale: String) -> ScanViewController? {
        
        return self.createViewController(withDelegate: delegate, configuration: ScanConfiguration(),locale: locale)
    }
    
    @objc static public func createViewController(withDelegate delegate: ScanDelegate? = nil, configuration: ScanConfiguration,locale: String) -> ScanViewController? {
    ...
    public func setStrings() {
        self.scanCardLabel.text = "Scan Card".localize(lang!)
        self.positionCardLabel.text = "SubTitle".localize(lang!)
    ...
    public override func viewDidLoad() {
        super.viewDidLoad()
        self.torchButton.layer.cornerRadius = 12.0
        self.torchButton.layer.borderWidth = 1
        if #available(iOS 13.0, *) {
            self.torchButton.layer.borderColor = CGColor(red: 0.878, green: 0.902, blue: 0.937, alpha: 1)
        } else {
    ...
    @IBAction func toggleTorch(_ sender: Any) {
        let imgName = isTorchOn() ? "flash_on" : "flash_off"
        self.toggleTorch()
        if #available(iOS 13.0, *) {
            let bundle = CSBundle.bundle()!
            if let image = UIImage(named: imgName,in: bundle, with: nil){
                self.torchButton.setImage(image, for: .normal)
            }
        } else {}
    }
